...
louvain::louvain(std::vector<int> adjacency, const size_t& vertices)
{
    N = vertices;
...
unsigned long louvain::getDegree(int index)
{
    long x = 0;
...
void louvain::populateA_valueMatrix()
{
    for(long i = 0; i < N; i++)
        aValues[i] = static_cast<double> (getDegree(i)) / static_cast<double> (twiceM);
}

double louvain::maxDeltaQ_InRow(int rowIndex, int& j)
{
...
void louvain::updateDeltaQMartix(int j, int i)
{
    int k = 0;
...
void louvain::updateAValuesArray(int j, int i)
{
    aValues[j] += aValues[i]; aValues[i] = 0;
}

void louvain::AlgorithmCNM()
{
    populateDeltaQ();
...
void louvain::displayCommunities()
{
